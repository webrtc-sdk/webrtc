...
// Ratio: float (0, +inf)
// Db: float (-inf, +inf)
RTC_EXPORT static inline float S16ToFloat(int16_t v) {
  constexpr float kScaling = 1.f / 32768.f;
  return v * kScaling;
}

RTC_EXPORT static inline int16_t FloatS16ToS16(float v) {
  v = std::min(v, 32767.f);
  v = std::max(v, -32768.f);
  return static_cast<int16_t>(v + std::copysign(0.5f, v));
}

RTC_EXPORT static inline int16_t FloatToS16(float v) {
  v *= 32768.f;
  v = std::min(v, 32767.f);
  v = std::max(v, -32768.f);
  return static_cast<int16_t>(v + std::copysign(0.5f, v));
...
RTC_EXPORT static inline float FloatToFloatS16(float v) {
  v = std::min(v, 1.f);
  v = std::max(v, -1.f);
  return v * 32768.f;
}

RTC_EXPORT static inline float FloatS16ToFloat(float v) {
  v = std::min(v, 32768.f);
  v = std::max(v, -32768.f);
  constexpr float kScaling = 1.f / 32768.f;
  return v * kScaling;
...
RTC_EXPORT void FloatToS16(const float* src, size_t size, int16_t* dest);
RTC_EXPORT void S16ToFloat(const int16_t* src, size_t size, float* dest);
RTC_EXPORT void S16ToFloatS16(const int16_t* src, size_t size, float* dest);
RTC_EXPORT void FloatS16ToS16(const float* src, size_t size, int16_t* dest);
RTC_EXPORT void FloatToFloatS16(const float* src, size_t size, float* dest);
RTC_EXPORT void FloatS16ToFloat(const float* src, size_t size, float* dest);

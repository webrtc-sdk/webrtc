...
@implementation RTC_OBJC_TYPE (RTCAudioProcessingConfig) {
  ...
  // `mobile_mode` was removed from webrtc::AudioProcessing::Config::EchoCanceller
  // in M150 (AEC3-only). Keep the public property for API compatibility, backed
  // by a standalone ivar that no longer maps to the native config.
  BOOL _echoCancellationMobileMode;
}
...
// echo_canceller.mobile_mode was removed in M150; kept as a no-op for API compat.

- (BOOL)isEchoCancellationMobileMode {
  return _echoCancellationMobileMode;
}

- (void)setIsEchoCancellationMobileMode:(BOOL)value {
  _echoCancellationMobileMode = value;
}

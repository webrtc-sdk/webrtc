...
static void overrideColorSpaceAttachments(CVImageBufferRef imageBuffer) {
  ...
  CVBufferSetAttachment(imageBuffer, (CFStringRef) @"ColorInfoGuessedBy",
                        (CFStringRef) @"RTCVideoDecoderH265", kCVAttachmentMode_ShouldPropagate);
}

std::span<const uint8_t> vpsDataFromHvcc(const uint8_t *hvccData, size_t hvccDataSize) {
  // Avoid copying: parse directly from the provided buffer.
  webrtc::BitstreamReader reader(
      absl::string_view(reinterpret_cast<const char *>(hvccData), hvccDataSize));
  ...
  for (uint32_t j = 0; j < numOfArrays; j++) {
    // NAL_unit_type: bit(1) array_completeness; unsigned int(1) reserved = 0; unsigned int(6)
    // NAL_unit_type;
    ...
    for (uint32_t k = 0; k < numOfNalus; k++) {
      ...
      // Position at the start of NAL unit payload in bytes.
      size_t nalStartPos = hvccDataSize - static_cast<size_t>(reader.RemainingBitCount() / 8);
      ...
      return {hvccData + nalStartPos + hevcNalHeaderSize, size - hevcNalHeaderSize};
    }
  }
  reader.Ok();
  return {};
...
uint8_t ComputeH265ReorderSizeFromAnnexB(const uint8_t* annexb_buffer, size_t annexb_buffer_size) {
    // FIXME: we should probably get the VPS from the SPS sps_video_parameter_set_id.
    webrtc::AnnexBBufferReader bufferReader(webrtc::MakeArrayView(annexb_buffer, annexb_buffer_size));
    if (!bufferReader.SeekToNextNaluOfType(webrtc::H265::kVps))
        return 0;

    static const size_t hevcNalHeaderSize = 2;
    webrtc::ArrayView<const uint8_t> nalu_data;
    if (!bufferReader.ReadNalu(nalu_data) || nalu_data.size() <= hevcNalHeaderSize)
        return 0;

    return ComputeH265ReorderSizeFromVPS(nalu_data.data() + hevcNalHeaderSize, nalu_data.size() - hevcNalHeaderSize);
}

// This is the callback function that VideoToolbox calls when decode is
// complete.
void h265DecompressionOutputCallback(void *decoderRef, void *params, OSStatus status,
...
CMSampleBufferRef H265BufferToCMSampleBuffer(const uint8_t *buffer, size_t buffer_size,
                                             CMVideoFormatDescriptionRef video_format)
    CF_RETURNS_RETAINED {
  CMBlockBufferRef new_block_buffer;
  if (auto error = CMBlockBufferCreateWithMemoryBlock(
          kCFAllocatorDefault, NULL, buffer_size, kCFAllocatorDefault, NULL, 0, buffer_size,
          kCMBlockBufferAssureMemoryNowFlag, &new_block_buffer)) {
    RTC_LOG(LS_ERROR)
  ...
  if (auto error =
          CMSampleBufferCreate(kCFAllocatorDefault, block_buffer.get(), true, nullptr, nullptr,
                               video_format, 1, 0, nullptr, 0, nullptr, &sample_buffer)) {
...
- (NSInteger)decode:(RTC_OBJC_TYPE(RTCEncodedImage) *)inputImage
  ...
  return [self decodeData:(uint8_t *)inputImage.buffer.bytes
                     size:inputImage.buffer.length
...
- (NSInteger)decodeData:(const uint8_t *)data size:(size_t)size timeStamp:(int64_t)timeStamp {
  ...
  if (!_useHEVC) {
    webrtc::ScopedCFTypeRef<CMVideoFormatDescriptionRef> inputFormat = webrtc::ScopedCF(webrtc::CreateH265VideoFormatDescription(webrtc::MakeArrayView((uint8_t*)data, size)));
    if (inputFormat) {
  ...
  if (_useHEVC) {
    ...
    if (!sampleBuffer) return WEBRTC_VIDEO_CODEC_ERROR;
          ...
          webrtc::MakeArrayView((uint8_t*)data, size),
...
- (NSInteger)setHVCCFormat:(const uint8_t *)data
                      size:(size_t)size
                     width:(uint16_t)width
                    height:(uint16_t)height {
  CFStringRef avcCString = (CFStringRef) @"hvcC";
  CFDataRef codecConfig = CFDataCreate(kCFAllocatorDefault, data, size);
  CFDictionaryRef atomsDict =
      CFDictionaryCreate(NULL, (const void **)&avcCString, (const void **)&codecConfig, 1,
                         &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
  CFDictionaryRef extensionsDict = CFDictionaryCreate(
      NULL, (const void **)&kCMFormatDescriptionExtension_SampleDescriptionExtensionAtoms,
      (const void **)&atomsDict, 1, &kCFTypeDictionaryKeyCallBacks,
      &kCFTypeDictionaryValueCallBacks);

  CMVideoFormatDescriptionRef videoFormatDescription = nullptr;
  auto err = CMVideoFormatDescriptionCreate(NULL, kCMVideoCodecType_HEVC, width, height,
                                            extensionsDict, &videoFormatDescription);
...
- (int)resetDecompressionSession {
  ...
  OSStatus status = VTDecompressionSessionCreate(nullptr, _videoFormat, nullptr, attributes,
...
- (void)flush {
  if (_decompressionSession) VTDecompressionSessionWaitForAsynchronousFrames(_decompressionSession);
...
- (void)processFrame:(RTC_OBJC_TYPE(RTCVideoFrame) *)decodedFrame
         reorderSize:(uint64_t)reorderSize {

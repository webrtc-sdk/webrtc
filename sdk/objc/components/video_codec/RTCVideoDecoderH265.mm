...
uint8_t ComputeH265ReorderSizeFromAnnexB(const uint8_t* annexb_buffer, size_t annexb_buffer_size) {
    // FIXME: we should probably get the VPS from the SPS sps_video_parameter_set_id.
    webrtc::AnnexBBufferReader bufferReader(std::span<const uint8_t>(annexb_buffer, annexb_buffer_size));
...
- (NSInteger)decodeData:(const uint8_t *)data size:(size_t)size timeStamp:(int64_t)timeStamp {
  ...
  if (!_useHEVC) {
    webrtc::ScopedCFTypeRef<CMVideoFormatDescriptionRef> inputFormat = webrtc::ScopedCF(webrtc::CreateH265VideoFormatDescription(std::span<const uint8_t>((const uint8_t*)data, size)));
    if (inputFormat) {
  ...
  if (_useHEVC) {
    ...
    if (!sampleBuffer) return WEBRTC_VIDEO_CODEC_ERROR;
          ...
          std::span<const uint8_t>((const uint8_t*)data, size),

...
bool H265AnnexBBufferToCMSampleBuffer(std::span<const uint8_t> annexb_buffer,
                                      CMVideoFormatDescriptionRef video_format,
                                      CMSampleBufferRef* out_sample_buffer) {
  RTC_DCHECK(out_sample_buffer);
  RTC_DCHECK(video_format);
  *out_sample_buffer = nullptr;
  ...
  AvccBufferWriter writer(std::span<uint8_t>(
      reinterpret_cast<uint8_t*>(data_ptr), block_buffer_size));
  while (reader.BytesRemaining() > 0) {
    std::span<const uint8_t> nalu_data;
    if (reader.ReadNalu(nalu_data)) {
      writer.WriteNalu(nalu_data);
...
uint8_t ComputeH264ReorderSizeFromAnnexB(std::span<const uint8_t> annexb_buffer) {
  AnnexBBufferReader reader(annexb_buffer);
  if (!reader.SeekToNextNaluOfType(kSps)) {
    return 0;
  }
  std::span<const uint8_t> sps_data;

  if (!reader.ReadNalu(sps_data) || sps_data.size() <= H264::kNaluTypeSize) {
    return 0;
  }

  std::vector<uint8_t> unpacked_buffer = H264::ParseRbsp(sps_data.data() + H264::kNaluTypeSize, sps_data.size() - H264::kNaluTypeSize);
...
CMVideoFormatDescriptionRef CreateH265VideoFormatDescription(
  ...
  AnnexBBufferReader reader(annexb_buffer);
  // Skip everything before the VPS, then read the VPS, SPS and PPS
  if (!reader.SeekToNextNaluOfType(H265::kVps)) {
    return nullptr;
  }
  std::span<const uint8_t> params_set;
  if (!reader.ReadNalu(params_set)) {
    RTC_LOG(LS_ERROR) << "Failed to read VPS";
    return nullptr;
  }
  param_set_ptrs[0] = params_set.data();
  param_set_sizes[0] = params_set.size();
  
  if (!reader.ReadNalu(params_set)) {
    RTC_LOG(LS_ERROR) << "Failed to read SPS";
    return nullptr;
  }
  param_set_ptrs[1] = params_set.data();
  param_set_sizes[1] = params_set.size();
  
  if (!reader.ReadNalu(params_set)) {
    RTC_LOG(LS_ERROR) << "Failed to read PPS";
    return nullptr;
  }
  param_set_ptrs[2] = params_set.data();
  param_set_sizes[2] = params_set.size();
...
bool AnnexBBufferReader::SeekToNextNaluOfType(H265::NaluType type) {
  for (; offset_ != offsets_.end(); ++offset_) {
    if (offset_->payload_size < 1)
      continue;
    if (H265::ParseNaluType(buffer_[offset_->payload_start_offset]) == type)
      return true;
  }
  return false;
}

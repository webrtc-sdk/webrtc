...
@implementation RTC_OBJC_TYPE (RTCCustomAudioSource) {
  webrtc::scoped_refptr<webrtc::CustomAudioSource> _nativeCustomAudioSource;
}

@synthesize nativeCustomAudioSource = _nativeCustomAudioSource;
...
- (instancetype)initWithFactory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory
  ...
  if (self) {
    _nativeCustomAudioSource = nativeSource;
  }
  return self;
}
...
- (int)sampleRate {
  return _nativeCustomAudioSource->sample_rate_hz();
}

- (NSUInteger)channels {
  return _nativeCustomAudioSource->num_channels();
}

- (int)queueSizeMs {
  return _nativeCustomAudioSource->queue_size_ms();
}

- (int64_t)bufferedDurationMs {
  return _nativeCustomAudioSource->BufferedDurationMs();
}
...
- (BOOL)captureFrameWithData:(NSData *)pcmData
  ...
  if (pcmData.length != frames * channels * sizeof(int16_t)) {
    RTC_LOG(LS_WARNING)
        << "RTCCustomAudioSource: data length " << pcmData.length
...
- (BOOL)captureAVAudioPCMBuffer:(AVAudioPCMBuffer *)pcmBuffer
  ...
  if (!ConvertPCMBufferToInterleavedS16(pcmBuffer, interleaved)) {
    RTC_LOG(LS_WARNING) << "RTCCustomAudioSource: unsupported "
...
- (BOOL)captureSampleBuffer:(CMSampleBufferRef)sampleBuffer
  ...
  if (status != noErr) {
    RTC_LOG(LS_WARNING) << "RTCCustomAudioSource: failed to copy PCM data "
...
- (void)clearBuffer {
  _nativeCustomAudioSource->ClearBuffer();
}
...
- (BOOL)captureBytes:(const int16_t *)data
  ...
  return _nativeCustomAudioSource->PushFrame(

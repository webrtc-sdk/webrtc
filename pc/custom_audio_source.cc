...
scoped_refptr<CustomAudioSource> CustomAudioSource::Create(
    int sample_rate_hz,
    size_t num_channels,
    int queue_size_ms,
  ...
  if (sample_rate_hz <= 0 || sample_rate_hz % 100 != 0 || num_channels == 0 ||
    ...
    RTC_LOG(LS_ERROR)
        << "CustomAudioSource: invalid arguments (sample rate must be a "
  ...
  auto source = make_ref_counted<CustomAudioSource>(
...
CustomAudioSource::CustomAudioSource(int sample_rate_hz,
                                     size_t num_channels,
                                     int queue_size_ms,
                                     TaskQueueFactory* task_queue_factory,
                                     Clock* clock)
  ...
  audio_queue_ = task_queue_factory->CreateTaskQueue(
      "CustomAudioSource", TaskQueueFactory::Priority::NORMAL);
...
void CustomAudioSource::AddSink(AudioTrackSinkInterface* sink) {
  RTC_DCHECK(sink);
  MutexLock lock(&mutex_);
...
void CustomAudioSource::RemoveSink(AudioTrackSinkInterface* sink) {
  {
    MutexLock lock(&mutex_);
...
bool CustomAudioSource::PushFrame(const int16_t* data,
                                  int sample_rate_hz,
                                  size_t num_channels,
                                  size_t samples_per_channel,
                                  absl::AnyInvocable<void() &&> on_complete) {
  if (sample_rate_hz != sample_rate_hz_ || num_channels != num_channels_) {
    RTC_LOG(LS_WARNING) << "CustomAudioSource: rejected frame with format "
  ...
  bool fire_inline = false;
  {
    ...
    if (queue_size_samples_ == 0) {
      ...
      if (samples_per_channel * 100 != static_cast<size_t>(sample_rate_hz_)) {
        RTC_LOG(LS_WARNING)
            << "CustomAudioSource: synchronous mode requires 10 ms frames, "
      ...
      ArrayView<const int16_t> samples(data, num_samples);
...
void CustomAudioSource::ClearBuffer() {
  absl::AnyInvocable<void() &&> completion;
  {
    MutexLock lock(&mutex_);
    buffer_.clear();
...
int64_t CustomAudioSource::BufferedDurationMs() const {
  MutexLock lock(&mutex_);
...
void CustomAudioSource::DeliverToSinks(
...
absl::AnyInvocable<void() &&> CustomAudioSource::TakeCompletionIfDue() {
  if (on_complete_ &&

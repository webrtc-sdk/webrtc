...
// independently of each other and of the ADM microphone capture path, and
// is_external_source() keeps AudioState from also pushing device audio into
// streams fed by this source.
//
...
class ExternalAudioSource : public LocalAudioSource {
 public:
  ...
  static scoped_refptr<ExternalAudioSource> Create(
      int sample_rate_hz,
      size_t num_channels,
      int queue_size_ms,
 ...
 protected:
  ExternalAudioSource(int sample_rate_hz,
                      size_t num_channels,
                      int queue_size_ms,
                      TaskQueueFactory* task_queue_factory,
                      Clock* clock);
 ...
 private:
  ...
  // notify_threshold_samples_ (2x the queue size, matching rust-sdks).
  ...
  // Destroyed first in ~ExternalAudioSource (explicitly reset) so no pacer

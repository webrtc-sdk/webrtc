...
class FakeSink : public AudioTrackSinkInterface {
 public:
  void OnData(const void* audio_data,
    ...
    ++frames_received_;
    ArrayView<const int16_t> samples(static_cast<const int16_t*>(audio_data),
...
class CustomAudioSourceTest : public ::testing::Test {
 protected:
  CustomAudioSourceTest()
      : time_controller_(Timestamp::Seconds(1)),
        task_queue_factory_(time_controller_.CreateTaskQueueFactory()) {}

  scoped_refptr<CustomAudioSource> CreateSource(int queue_size_ms) {
    return CustomAudioSource::Create(
...
TEST_F(CustomAudioSourceTest, CreateRejectsInvalidArguments) {
  EXPECT_EQ(
      CustomAudioSource::Create(0, kNumChannels, 0, task_queue_factory_.get()),
      nullptr);
  EXPECT_EQ(CustomAudioSource::Create(44110, kNumChannels, 0,
                                      task_queue_factory_.get()),
            nullptr);
  EXPECT_EQ(
      CustomAudioSource::Create(kSampleRate, 0, 0, task_queue_factory_.get()),
      nullptr);
  EXPECT_EQ(CustomAudioSource::Create(kSampleRate, kNumChannels, 15,
                                      task_queue_factory_.get()),
            nullptr);
  EXPECT_EQ(CustomAudioSource::Create(kSampleRate, kNumChannels, 100,
                                      /*task_queue_factory=*/nullptr),
            nullptr);
}

TEST_F(CustomAudioSourceTest, SourceTypeIsCustom) {
  auto source = CreateSource(/*queue_size_ms=*/0);
  EXPECT_EQ(source->source_type(), AudioSourceInterface::SourceType::kCustom);
  // The base local source must stay on the ADM path.
  auto local = LocalAudioSource::Create(nullptr);
  EXPECT_EQ(local->source_type(),
            AudioSourceInterface::SourceType::kAudioDeviceModule);
}

TEST_F(CustomAudioSourceTest, AppliesAudioOptions) {
  AudioOptions options;
  options.echo_cancellation = false;
  auto source = CustomAudioSource::Create(
...
TEST_F(CustomAudioSourceTest, SyncModeDeliversToAllSinks) {
  auto source = CreateSource(/*queue_size_ms=*/0);
  FakeSink sink1;
  FakeSink sink2;
...
TEST_F(CustomAudioSourceTest, SyncModeRejectsNon10MsFrames) {
  auto source = CreateSource(/*queue_size_ms=*/0);
  FakeSink sink;
  source->AddSink(&sink);
...
TEST_F(CustomAudioSourceTest, RejectsMismatchedFormat) {
...
TEST_F(CustomAudioSourceTest, BufferedModePacesDeliveryIn10MsFrames) {
  auto source = CreateSource(kQueueSizeMs);
  FakeSink sink;
  source->AddSink(&sink);
...
TEST_F(CustomAudioSourceTest, BufferedModeFlushesSubFrameResidual) {
  auto source = CreateSource(kQueueSizeMs);
  FakeSink sink;
  source->AddSink(&sink);
...
TEST_F(CustomAudioSourceTest, BufferedModeFeedsSilenceOnUnderrun) {
  auto source = CreateSource(kQueueSizeMs);
  FakeSink sink;
  source->AddSink(&sink);
...
TEST_F(CustomAudioSourceTest, BufferedDurationIsExactForNonMultipleRates) {
...
TEST_F(CustomAudioSourceTest, BufferedModeRejectsWhenFull) {
  auto source = CreateSource(kQueueSizeMs);
...
TEST_F(CustomAudioSourceTest, BufferedModeCompletionFiresOnDrain) {
  auto source = CreateSource(kQueueSizeMs);
  FakeSink sink;
  source->AddSink(&sink);
...
TEST_F(CustomAudioSourceTest, BufferedModeCompletionFiresInlineBelowThreshold) {
  auto source = CreateSource(kQueueSizeMs);
...
TEST_F(CustomAudioSourceTest, CompletionHandlerMayPushNextFrame) {
  auto source = CreateSource(kQueueSizeMs);
  FakeSink sink;
  source->AddSink(&sink);
...
TEST_F(CustomAudioSourceTest, ClearBufferDropsAudioAndFiresCompletion) {
  auto source = CreateSource(kQueueSizeMs);
...
TEST_F(CustomAudioSourceTest, DestructionFlushesPendingCompletion) {
  auto source = CreateSource(kQueueSizeMs);
...
TEST(CustomAudioSourceRealTimeTest, RemoveSinkFencesInFlightDelivery) {
...
TEST_F(CustomAudioSourceTest, DestructionWhilePacerRunningIsSafe) {
  auto source = CreateSource(kQueueSizeMs);
  FakeSink sink;
  source->AddSink(&sink);
